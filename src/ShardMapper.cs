...
//			}
//			return (TModel)SqlRdrDelegate(shardId, parameters, logger);
//		}
...
//			}
//			return (List<TModel>)SqlRdrDelegate(shardId, rdr, logger);
//		}
...
//		{
//			ParameterExpression prmShardId = Expression.Variable(typeof(TShard), "shardId");
...
//			var expressionPrms = new ParameterExpression[] { prmShardId, expPrms, expLogger };
...
//            };
//			//Loop through all object properties:
//			IterateGetOutParameters(TModel, prmShardId, expPrms, expPrm, variableExpressions, expressions, expModel, expLogger, logger);
...
//		}
//		private static void IterateGetOutParameters(Type TModel, ParameterExpression prmShardId, ParameterExpression expPrms, ParameterExpression expPrm, List<ParameterExpression> variableExpressions, List<Expression> expressions, Expression expModel, ParameterExpression expLogger, ILogger logger)
//		{
...
//				{
//					var attrPMs = prop.GetCustomAttributes<ParameterMapAttribute>(true);
//					//ParameterExpression expVarShardId = null;
...
//					//	}
//					//	expVarShardId = Expression.Variable(shardTypes[0], prop.Name + "_shardId");
//					//	variableExpressions.Add(expVarShardId);
//					//	expressions.Add(Expression.Assign(expVarShardId, prmShardId));
...
//						//		}
//						//		attrPM.AppendReadOutParameterExpressions(prmShardId, expVarShardId, expressions, expPrms, expPrm, prop, attrPM.ShardPosition, expLogger, logger);
//						//		break;
...
//						//		}
//						//		attrPM.AppendReadOutParameterExpressions(prmShardId, expVarRecordId, expressions, expPrms, expPrm, prop, attrPM.ShardPosition, expLogger, logger);
//						//		break;
...
//						//		}
//						//		attrPM.AppendReadOutParameterExpressions(prmShardId, expVarChildId, expressions, expPrms, expPrm, prop, attrPM.ShardPosition, expLogger, logger);
//						//		break;
...
//						//		}
//						//		attrPM.AppendReadOutParameterExpressions(prmShardId, expVarConcurrency, expressions, expPrms, expPrm, prop, attrPM.ShardPosition, expLogger, logger);
//						//		break;
//						//	default:
//						attrPM.AppendReadOutParameterExpressions(expProperty, expressions, expPrms, expPrm, prop.ReflectedType, expLogger, logger);
//						//		break;
//						//}
//					}
//					//if (isPropShardKey)
...
//					//	Expression.AndAlso(Expression.NotEqual(expVarShardId, Expression.Constant(null)),
//					//		Expression.NotEqual(expVarRecordId, Expression.Constant(null))),
//					//	//true
//					//	Expression.Assign(expProperty, Expression.New(prop.PropertyType.GetConstructor(new Type[] { typeof(DataOrigin), shardTypes[0], shardTypes[1], typeof(DateTime?) }), new Expression[] { expDataOrigin, expVarShardId, expVarRecordId, expVarConcurrency })),
//					//	//false
...
//					//			{ Expression.Property(expVarShardId, shardTypes[1].GetProperty(nameof(Nullable<int>.Value), 
//					//		expVarShardId, typeof(byte?).GetProperty(nameof(Nullable<int>.Value), tree })),
//					//	Expression.Assign(expProperty, Expression.Constant(null)));
...
//				{
//					MemberExpression expProperty = Expression.Property(expModel, prop);
//					IterateGetOutParameters(prop.PropertyType, prmShardId, expPrms, expPrm, variableExpressions, expressions, expProperty, expLogger, logger);
//				}
//			}
//		}
...
//			ParameterExpression expLogger = Expression.Parameter(typeof(ILogger), "logger");
//			var expressionPrms = new ParameterExpression[] { prmShardId, prmSqlRdr, expLogger };
...
//			//Loop through all object properties:
//			IterateRdrColumns(TModel, prmShardId, expModel, columnLookupExpressions, expressions, prmSqlRdr, expOrdinals, expOrdinal, ref propIndex, expLogger, logger);
...
//			return lambda.Compile();
//		}
//		private static void IterateRdrColumns(Type TModel, ParameterExpression prmShardId, Expression expModel, List<MethodCallExpression> columnLookupExpressions, List<Expression> expressions, ParameterExpression prmSqlRdr, ParameterExpression expOrdinals, ParameterExpression expOrdinal, ref int propIndex, ParameterExpression expLogger, ILogger logger)
//		{
...
//				{
//					MemberExpression expProperty = Expression.Property(expModel, prop);
//					IterateRdrColumns(prop.PropertyType, prmShardId, expProperty, columnLookupExpressions, expressions, prmSqlRdr, expOrdinals, expOrdinal, ref propIndex, expLogger, logger);
//				}
//			}
...
//			=> SqlResultsHandler<TShard, TReaderResult, Mapper.DummyType, Mapper.DummyType, Mapper.DummyType, Mapper.DummyType, Mapper.DummyType, Mapper.DummyType, Mapper.DummyType, TOutParameters, TResult>(dataOrigin, shardId, proceedureName, rdr, parameters, logger);
...
//			if (rdr is null)
//			{
//				logger.LogError($"Null reader object provided to query handler for {procedureName} on shard {shardId.ToString()}, returning null result.");
//				return null;
//			}
//			if (rdr.IsClosed)
//			{
//				logger.LogError($"Data reader object was closed for {procedureName} on shard {shardId.ToString()}, returning null result.");
//				return null;
//			}
...
//			{
//				resultList0 = ShardMapper.FromDataReader<TShard, TReaderResult0>(shardId, rdr, logger);
//				hasNextResult = rdr.NextResult();
//			}
//			if (hasNextResult && typeof(TReaderResult1) != dummy)
//			{
//				resultList1 = ShardMapper.FromDataReader<TShard, TReaderResult1>(shardId, rdr, logger);
//				hasNextResult = rdr.NextResult();
//			}
//			if (hasNextResult && typeof(TReaderResult2) != dummy)
//			{
//				resultList2 = ShardMapper.FromDataReader<TShard, TReaderResult2>(shardId, rdr, logger);
//				hasNextResult = rdr.NextResult();
//			}
//			if (hasNextResult && typeof(TReaderResult3) != dummy)
//			{
//				resultList3 = ShardMapper.FromDataReader<TShard, TReaderResult3>(shardId, rdr, logger);
//				hasNextResult = rdr.NextResult();
//			}
//			if (hasNextResult && typeof(TReaderResult4) != dummy)
//			{
//				resultList4 = ShardMapper.FromDataReader<TShard, TReaderResult4>(shardId, rdr, logger);
//				hasNextResult = rdr.NextResult();
//			}
//			if (hasNextResult && typeof(TReaderResult5) != dummy)
//			{
//				resultList5 = ShardMapper.FromDataReader<TShard, TReaderResult5>(shardId, rdr, logger);
//				hasNextResult = rdr.NextResult();
//			}
//			if (hasNextResult && typeof(TReaderResult6) != dummy)
//			{
//				resultList6 = ShardMapper.FromDataReader<TShard, TReaderResult6>(shardId, rdr, logger);
//				hasNextResult = rdr.NextResult();
//			}
//			if (hasNextResult && typeof(TReaderResult7) != dummy)
//			{
//				resultList7 = ShardMapper.FromDataReader<TShard, TReaderResult7>(shardId, rdr, logger);
//				hasNextResult = rdr.NextResult();
//			}
//			if (typeof(TOutParameters) != dummy)
//			{
//				resultOutPrms = ShardMapper.ReadOutParameters<TShard, TOutParameters>(parameters, shardId, logger);
//			}
...
//			return (TResult)sqlObjectDelegate(shardId, procedureName, resultList0, resultList1, resultList2, resultList3, resultList4, resultList5, resultList6, resultList7, resultOutPrms, logger);
//		}
//		private static TResult AssignRootToResult<TShard, TEval, TResult>(TShard shardId, string procedureName, IList<TEval> resultList, ILogger logger) where TResult : class, new() where TEval : class, new() where TShard: IComparable
//		{
//			if (resultList is null)
//			{
//				logger.LogError($"Procedure {procedureName} on shard {shardId.ToString()} failed to return an expected base recordset result.");
//				return null;
//			}
//			else if (resultList.Count == 0)
//			{
//				logger.LogDebug($"Procedure {procedureName} on shard {shardId.ToString()} returned an empty base result.");
//				return null;
//			}
//			else if (resultList.Count > 1)
//			{
//				logger.LogError($"Procedure {procedureName} on shard {shardId.ToString()} returned multiple base recordset results.");
//				return null;
//			}
//			else
//			{
...
//				{
//					//result = AssignRootToResult<TReaderResult0, TResult>(shardId, procedureName, resultList0, logger)
//					expressions.Add(Expression.Assign(expResult, Expression.Call(miSetResultSetGeneric.MakeGenericMethod().MakeGenericMethod(new Type[] { typeof(TReaderResult0), resultType }), expShardId, expProcName, expResultSet0, expLogger)));
...
//				{
//					expressions.Add(Expression.Assign(expResult, Expression.Call(miSetResultSetGeneric.MakeGenericMethod().MakeGenericMethod(new Type[] { typeof(TReaderResult1), resultType }), expShardId, expProcName, expResultSet1, expLogger)));
...
//				{
//					expressions.Add(Expression.Assign(expResult, Expression.Call(miSetResultSetGeneric.MakeGenericMethod().MakeGenericMethod(new Type[] { typeof(TReaderResult2), resultType }), expShardId, expProcName, expResultSet2, expLogger)));
//					expressions.Add(Expression.IfThen(Expression.Equal(expResult, expNull), Expression.Return(expExitTarget, expNull, typeof(TResult))));
//					isRdrResult2Used = true;
//				}
//				else if (result is TReaderResult3)
//				{
//					expressions.Add(Expression.Assign(expResult, Expression.Call(miSetResultSetGeneric.MakeGenericMethod().MakeGenericMethod(new Type[] { typeof(TReaderResult3), resultType }), expShardId, expProcName, expResultSet3, expLogger)));
//					expressions.Add(Expression.IfThen(Expression.Equal(expResult, expNull), Expression.Return(expExitTarget, expNull, typeof(TResult))));
//					isRdrResult3Used = true;
//				}
//				else if (result is TReaderResult4)
//				{
//					expressions.Add(Expression.Assign(expResult, Expression.Call(miSetResultSetGeneric.MakeGenericMethod().MakeGenericMethod(new Type[] { typeof(TReaderResult4), resultType }), expShardId, expProcName, expResultSet4, expLogger)));
//					expressions.Add(Expression.IfThen(Expression.Equal(expResult, expNull), Expression.Return(expExitTarget, expNull, typeof(TResult))));
//					isRdrResult4Used = true;
//				}
//				else if (result is TReaderResult5)
//				{
//					expressions.Add(Expression.Assign(expResult, Expression.Call(miSetResultSetGeneric.MakeGenericMethod().MakeGenericMethod(new Type[] { typeof(TReaderResult5), resultType }), expShardId, expProcName, expResultSet5, expLogger)));
//					expressions.Add(Expression.IfThen(Expression.Equal(expResult, expNull), Expression.Return(expExitTarget, expNull, typeof(TResult))));
//					isRdrResult5Used = true;
//				}
//				else if (result is TReaderResult6)
//				{
//					expressions.Add(Expression.Assign(expResult, Expression.Call(miSetResultSetGeneric.MakeGenericMethod().MakeGenericMethod(new Type[] { typeof(TReaderResult6), resultType }), expShardId, expProcName, expResultSet6, expLogger)));
//					expressions.Add(Expression.IfThen(Expression.Equal(expResult, expNull), Expression.Return(expExitTarget, expNull, typeof(TResult))));
//					isRdrResult6Used = true;
//				}
//				else if (result is TReaderResult7)
//				{
//					expressions.Add(Expression.Assign(expResult, Expression.Call(miSetResultSetGeneric.MakeGenericMethod().MakeGenericMethod(new Type[] { typeof(TReaderResult7), resultType }), expShardId, expProcName, expResultSet7, expLogger)));
//					expressions.Add(Expression.IfThen(Expression.Equal(expResult, expNull), Expression.Return(expExitTarget, expNull, typeof(TResult))));
...
//								Expression.Call(miLogError, Expression.Call(miFormat, Expression.Constant($"Could not set property {prop.Name} because procedure {procedureName} on shard {{0}} unexpectedly returned {{1}} results instead of one."), expShardId, expCount)))
//								));
//						isRdrResult0Used = true;
//					}
...
//								Expression.Call(miFormat, Expression.Constant($"Could not set property {prop.Name} because procedure {procedureName} on shard {{0}} unexpectedly returned {{1}} results instead of one."), expShardId, expCount)))));
//						isRdrResult0Used = true;
//					}
...
//				(expBlock, new ParameterExpression[] { expShardId, expProcName, expResultSet0, expResultSet1, expResultSet2, expResultSet3, expResultSet4, expResultSet5, expResultSet6, expResultSet7, expPrmOut, expLogger }); //+parameters
//			return lambda.Compile();
//		}
//		#endregion
//	}
